...
from .forms import ParkingAssignmentQuickForm
...
class SpotListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        assigned = ParkingAssignment.objects.filter(end_date__isnull=True).values("spot").distinct().count()
        ctx["cnt_total"] = total
        ctx["cnt_assigned"] = assigned
        ctx["cnt_unassigned"] = total - assigned
        return ctx
