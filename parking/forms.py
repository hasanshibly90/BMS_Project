...
class VehicleForm(forms.ModelForm):
    ...
    # Optional quick parking assignment on save
    ...
    def clean(self):
        ...
        ext = cleaned.get("external_owner")

        # ensure exactly one matches owner_type
        if ot == Vehicle.OWNER and not owner:
            self.add_error("owner", "Select an Owner.")
        if ot == Vehicle.LESSEE and not lessee:
            self.add_error("lessee", "Select a Lessee.")
        if ot in (Vehicle.UBER_DRIVER, Vehicle.RENTAL_COMPANY) and not ext:
...
class ParkingSpotForm(forms.ModelForm):
    class Meta:
        model = ParkingSpot
        fields = ["code", "level", "is_reserved", "notes"]
