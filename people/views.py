...
def _try_image(c, path, x, y, w, h):
    try:
        if path: c.drawImage(path, x, y, width=w, height=h, preserveAspectRatio=True, anchor='n')
    except Exception:
        pass

class OwnerListView(ListView):
    model = Owner; template_name = "people/owner_list.html"; paginate_by = 30
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs); ctx["q"] = (self.request.GET.get("q") or "").strip(); return ctx

class OwnerCreateView(CreateView):
    model = Owner; form_class = OwnerForm; template_name = "form.html"; success_url = reverse_lazy("people:owners")
...
def owner_pdf(request, pk):
    ...
    c = canvas.Canvas(resp, pagesize=A4); W, H = A4; margin = 18 * mm; x = margin; y = H - margin
    c.setFont("Helvetica-Bold", 16); c.drawString(x, y, "OWNER PROFILE")
    c.setFont("Helvetica", 9); c.drawRightString(W - margin, y, timezone.now().strftime("%d-%b-%Y %H:%M")); y -= 14 * mm
    photo_path = getattr(getattr(obj, "photo", None), "path", None)
    nid_path = getattr(getattr(obj, "nid_image", None), "path", None)
    ...
    y -= 5 * mm; c.setFont("Helvetica-Bold", 11); c.drawString(x, y, "Ownership history"); y -= 6 * mm
    c.setFont("Helvetica", 10)
    owns = Ownership.objects.filter(owner=obj).order_by("-start_date")
    if not owns: c.drawString(x, y, "(no ownership records)"); y -= line
    for o in owns:
        flat = f"{o.flat.unit}-{o.flat.floor:02d}"; span = f"{o.start_date} to {o.end_date or 'present'}"
        c.drawString(x, y, _safe(f"{flat} | {span}")); y -= line
        if y < margin + 20 * mm: c.showPage(); y = H - margin
    c.showPage(); c.save(); return resp

class LesseeListView(ListView):
    model = Lessee; template_name = "people/lessee_list.html"; paginate_by = 30
    def get_queryset(self):
...
class LesseeCreateView(CreateView):
    model = Lessee; form_class = LesseeForm; template_name = "form.html"; success_url = reverse_lazy("people:lessees")
...
def lessee_pdf(request, pk):
    ...
    _draw_label_value(c, x, y, "Email:", obj.email); y -= line
    _draw_label_value(c, x, y, "Address:", obj.address); y -= line
    y -= 5 * mm; c.setFont("Helvetica-Bold", 11); c.drawString(x, y, "Tenancy history"); y -= 6 * mm
    c.setFont("Helvetica", 10)
    tens = Tenancy.objects.filter(lessee=obj).order_by("-start_date")
    if not tens: c.drawString(x, y, "(no tenancy records)"); y -= line
    for t in tens:
        flat = f"{t.flat.unit}-{t.flat.floor:02d}"; span = f"{t.start_date} to {t.end_date or 'present'}"
        c.drawString(x, y, _safe(f"{flat} | {span}")); y -= line
        if y < margin + 20 * mm: c.showPage(); y = H - margin
    c.showPage(); c.save(); return resp
